defmodule WhereCorro.SandwichSender do
  use GenServer
  require Logger

  def start_link(_opts \\ []) do
    # This is the function that gets run by the supervisor when I run the server
    GenServer.start_link(WhereCorro.SandwichSender, [])
  end
  ...
  def init(_opts) do
    ...
    vm = Application.fetch_env!(:where_corro, :fly_vm_id)
    IO.inspect("About to call init local sandwich #{vm}")
    init_local_sandwich(vm)
    IO.inspect("About to start a watch on the sw table")
    WhereCorro.CorroCalls.start_watch({"watchwich", "SELECT pk AS vm_id, sandwich FROM sw"})
    IO.inspect("About to start a watch on the sandwich_services table")
    WhereCorro.CorroCalls.start_watch({"services", "SELECT vm_id, region, srv_state FROM sandwich_services"})
    {:ok, []}
  end
  ...
  def handle_info({:sandwich, message}, state) do
    ...
    vm = Application.fetch_env!(:where_corro, :fly_vm_id)
    WhereCorro.FlyDnsReq.get_all_instances()
  ...
  def get_local_sandwich(vm_id) do
    ...
    WhereCorro.CorroCalls.query_corro(statement)
  end
  ...
  def get_sandwich_table() do
    ...
    WhereCorro.CorroCalls.corro_request("query", statement)
    |> IO.inspect()
  end
