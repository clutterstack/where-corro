defmodule WhereCorroWeb.Components.PropagationMap do
  ...
  @h 320
  @fallback_radius 150
  @fallback_center {400, 195}

  attr :h, :integer, default: @h
  attr :w, :integer, default: @w
  ...
  attr :node_regions, :map, default: %{}

  def propagation_map(assigns) do
    assigns = assign_computed_data(assigns)
    ...
    >
      <defs>
        <%= for {id, color} <- [
              {"pendingGradient", "#fbbf24"},
              {"ackGradient", "#34d399"},
              {"failedGradient", "#f87171"},
              {"localGradient", "#60a5fa"}
            ] do %>
    ...
    <!-- World map SVG paths placeholder - replace with your actual paths -->
      <g id="world_map_paths">
        <g id="ne_110m_ocean">
          <path
            d="M 508.86 108.99 503.48 101.62 513.48 96.21 517.58 96.64 517.57 100.18 511.51 101.61 513.81 104.89 521.33 109.73 517.3 109.89 519.31 118.56 512.7 118.77 509.06 117.19 508.86 108.99 Z"
            stroke="#DAA520"
            stroke-width="1"
            fill="#444444"
            fill-rule="evenodd"
      ...
      <%= for connection <- @connections do %>
        <g>
          ...
          <%= if Map.has_key?(connection, :latency_label) and connection.latency_label do %>
            <text
              x={connection.latency_label.x}
              y={connection.latency_label.y}
              text-anchor="middle"
              class="latency-label"
            >
    ...
    <!-- Node circles -->
      <%= for node <- @nodes do %>
        <g transform={"translate(#{node.x}, #{node.y})"}>
          ...
          <text y="-18" text-anchor="middle" font-size="14" font-weight="bold" fill="#374151">
            {node.label}
          </text>
          <text y="25" text-anchor="middle" font-size="12" fill="#6b7280">
            {node.region}
          </text>
        </g>
      <% end %>
    ...
    <!-- Legend -->
      <g transform="translate(10, 10)">
        ...
        <%= for {label, gradient, offset} <- [
              {"Local Node", "localGradient", 35},
              {"Pending", "pendingGradient", 55},
              {"Acknowledged", "ackGradient", 75},
              {"Failed", "failedGradient", 95}
            ] do %>
  ...
  defp assign_computed_data(assigns) do
    local_coords = get_node_coordinates(assigns.local_node_id, assigns.node_regions)

    connections =
      if local_coords do
        generate_connections(assigns.node_statuses, assigns.node_regions, local_coords)
      else
        []
      end

    all_statuses = Map.put(assigns.node_statuses, assigns.local_node_id, %{status: :local})
    nodes = generate_nodes(all_statuses, assigns.node_regions)

    assign(assigns, connections: connections, nodes: nodes)
  end
      ...
      case get_node_coordinates(node_id, node_regions) do
        ...
        node_coords ->
          ...
          base_connection = %{
            ...
            x2: x2,
            y2: y2,
            color: connection_color(status.status),
