defmodule WhereCorroWeb.ShowOutputLive do
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen overflow-hidden bg-gray-100 py-6 px-6">
      <%= if Application.fetch_env!(:where_corro, :fly_app_name) do %>
...
...
    ...
    <%= unless Application.fetch_env!(:where_corro, :corro_builtin)== "1" do %>
        ...
        <div>
          <code>The <%=Application.fetch_env!(:where_corro, :fly_corrosion_app)%> corrosion cluster is running in:
        ...
        <div>
        <code>top1.nearest.of.<%=Application.fetch_env!(:where_corro, :fly_corrosion_app)%>.internal: <%= @nearest_corrosion["ip"] %> (id <%= @nearest_corrosion["instance"] %>, in <%= @nearest_corrosion["region"] %>)</code>
        </div>
        </div>
      <% end %>
    </div>
  ...
  def mount(_params, _session, socket) do
    Phoenix.PubSub.subscribe(WhereCorro.PubSub, "from_corro")
    Phoenix.PubSub.subscribe(WhereCorro.PubSub, "sandwichmsg")
    Phoenix.PubSub.subscribe(WhereCorro.PubSub, "friend_regions")
    Phoenix.PubSub.subscribe(WhereCorro.PubSub, "corro_regions")
    Phoenix.PubSub.subscribe(WhereCorro.PubSub, "nearest_corrosion")
    # Phoenix.PubSub.subscribe(WhereCorro.PubSub, "corrosion_ip")
    init_app_regions(socket)
    with corro_app = Application.fetch_env(:where_corro, :fly_corrosion_app) do
      assign(socket,
  ...
  defp init_app_regions(socket) do
    case WhereCorro.FriendFinder.check_regions() do
